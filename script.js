...
let getCrimeData = async function () {
  try {
    ...
    svg_One
      ...
      .attr("x", (d, i) => {
        return (
          i * ((svgWidth - 60) / dataset_one_final.length) + yAxisLabelPadAmount
        );
      })
      .attr("y", (d) => {
        return (
          svgHeight - svgPadding - Math.abs(y_scale_one(d[1]) - y_scale_one(0))
        );
      })
      .attr(
        "width",
        (svgWidth - yAxisLabelPadAmount) / dataset_one_final.length - barPadding
      )
      .attr("height", (d) => {
      ...
      .on("mouseout", function (d) {
        d3.select(this).transition().duration(250).attr("fill", "green");
      })
      .append("title")
      .text(function (d) {
